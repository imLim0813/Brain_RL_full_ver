...
# GPU device
torch.cuda.set_device('cuda:0') if torch.cuda.is_available() else None
device = 'cpu'
...
class DDPG(object):
    ...
    def update(self, b_size, g_reward, epoch):
        ...
        for i in range(200):
            ...
            self.actor_iter += 1
            self.critic_iter += 1
        print(c_loss_mean/200)
        print(a_loss_mean/200)
...
def main():
    ...
    if mode == 'test':
        ...
        for episode in range(max_episode):
            ...
            for t in count():
                action_r, action_theta = agent.select_action(state)
                action_r = (action_r + np.random.normal(0, 0.01, size=1)).clip(0, 2)
